...
% 计算频谱
[spectrum, frequency] = mtspectrumc(icasig', params);
% 计算低频谱功率
low_freq_power = trapz(frequency, spectrum); % 对频谱进行积分，得到低频谱功率

% 对低频谱功率进行排序并找到前20个
[sorted_power, sorted_index] = sort(low_freq_power, 'descend');
top_IC_indices = sorted_index(1:50);

icasig = icasig(top_IC_indices,:);
% icasig_select = icasig(top_IC_indices,:);
A = A(:,top_IC_indices);
skew = skew(:,top_IC_indices);

W = pinv(A);

% save in matlab format
icaout = [sesInfo.ica_mat_file, '_sel'];
...
icatb_save([icaout,'.mat'], 'W', 'icasig', 'mask_ind', 'skew','A');
