...
def HDLFCA(Config):
    ...
    metric = {"accuracy": acc, "precision": spe, "sensitivity": sen, "F1_score": f1, "auc": roc_auc}
...
if __name__ == '__main__':
    ...
    seeds = np.arange(10)#生成0~9,10个数的一维数组，作为随机种子，重复十次实验
    metricss = []
    for i, seed in enumerate(seeds):
        Config.random_state = seed
        print("Running the model with seed : {}".format(seed))
        metric = HDLFCA(Config)
        metricss.append(metric)

    meanMetric, stdMetric = metricSummer(metricss)
    print("\n \ n meanMetrics_all : {}".format(meanMetric))
    print("stdMetric_all : {}".format(stdMetric))