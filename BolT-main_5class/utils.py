...
def metricSummer(metricss, type):
    ...
    for metrics in metricss: # this is over different seeds
        ...
        for key in meanMetric:
            stdMetric[key] = np.std(meanMetric[key])     #10fold中求均值和标准差
...
def calculateMetric(result):
    ...
    if(isMultiClass):  #多分类
        ...
        if(hasProbs):
            #我要做5分类，但可能test的数据只有四类，自动识别出数据中缺少的类别，并相应地调整模型输出和标签，以便进行 ROC AUC 计算。
            num_classes = 5

            # 确定原始标签中的所有类别
            original_classes = np.unique(labels)

            # 找到缺失的类别
            missing_label = np.setdiff1d(np.arange(num_classes), original_classes)

            # 调整概率矩阵
            adjusted_probs = np.delete(probs, missing_label, axis=1)

            # 计算 ROC AUC 分数
            roc = skmetr.roc_auc_score(labels, adjusted_probs, average="macro", multi_class="ovr")

            # # 确定真实的类别数量
            # true_num_classes = len(np.unique(labels))
            #
            # # 检测缺失的类别
            # missing_label = None
            # if true_num_classes < num_classes:
            ...
            #     adjusted_probs = np.insert(probs, missing_label, 0, axis=1)
            # else:
            #     adjusted_labels = labels
            #     adjusted_probs = probs
            #
            # # 计算 ROC AUC 分数
            # roc = skmetr.roc_auc_score(adjusted_labels, adjusted_probs, average="macro", multi_class="ovr")

            # # 计算 ROC AUC 分数
            #
            # roc = skmetr.roc_auc_score(labels, probs, average="macro", multi_class="ovr")  #得到AUC分数
...
def calculateMetrics(resultss):
    ...
    for results in resultss:#5次重复实验
        
        metrics = []

        for result in results:  #每一个fold
