...
def visualize_axial(saveDir, roiFileArray, colors): #给他16个图合起来？
...
if(targetDataset == "HCP_TASK"):
    ...
    targetRoiFiles = [filee for filee in glob(sourceFolder + "/*") if "nii.gz" in filee] #筛选条件是文件名包含 "nii.gz" 且与 targetClass 的标签匹配
...
for roiFile in targetRoiFiles:
    ...
    if(targetDataset == "HCP_REST"):
        ...
        isAsd = roiFile.split("/")[-1].split("_")[1] == "0"  #分离了class，即对应label
        if(isAsd):
            color = "4hot"
        else:
            color = "electric_blue"
...
for i, filee in enumerate(targetRoiFiles): #按标签给ROI分两类
...
visualize_sagittal(saveDir, leftRoiFiles, rightRoiFiles, leftColors, rightColors) #矢状位
